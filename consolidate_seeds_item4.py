#!/usr/bin/env python3
"""
Consolida os CSVs por-seed da GRADE DO ITEM 4 em dois arquivos enxutos.
...
nome do arquivo, e cls fica fixo em 20. Os nomes agora sao:
  ...
  treino:   <model>_cls<w>_<arm>_seed<s>_<ts>.csv
            ex: multimodal_cls20_agent_seed0_2026-08-19_04-54-06.csv
  metricas: metrics_<model>_cls<w>_<arm>_seed<s>[_<ts>].csv
            ex: metrics_multimodal_cls20_sdc_seed0_2026-08-19.csv

Como o regex agora EXIGE o braco, os arquivos da Parte A (sem braco) caem no
'ignored' de proposito -- nao se deve misturar os dois regimes (2 canais SDC vs
6 canais) na mesma tabela. Para reconsolidar a Parte A, use a versao antiga.

Saidas:
  1. metrics_all.csv    (mantem os 9 breakdowns por arquivo -> 32 x 9 = 288 linhas)
  2. train_summary.csv  (1 linha-resumo por run -> 32 linhas)

Uso:
    python3 consolidate_seeds_item4.py \
...
RE_TRAIN = re.compile(
    r"^(?P<model>multimodal|sequential)_cls(?P<cls>\d+)_(?P<arm>sdc|agent)"
    r"_seed(?P<seed>\d+)(?:_(?P<ts>" + _TS + r"))?\.csv$"
)
RE_METRICS = re.compile(
    r"^metrics_(?P<model>multimodal|sequential)_cls(?P<cls>\d+)_(?P<arm>sdc|agent)"
    r"_seed(?P<seed>\d+)(?:_(?P<ts>" + _TS + r"))?\.csv$"
)
...
def main():
    ...
    ap.add_argument("--expected", type=int, default=32,
                    help="numero esperado de runs por tipo (item 4: 2 modelos x "
                         "2 bracos x 8 seeds = 32)")
    args = ap.parse_args()
