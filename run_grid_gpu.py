#!/usr/bin/env python3
"""
Item-4 grid, GPU phase (train -> infer) for all combos, run IN THE GPU
CONTAINER. The metrics phase (validate_motion_official) runs separately in the
CPU/TF container -- see run_grid_validate.py.

Grid (opcao C): {multimodal, sequential} x {sdc, agent} x seeds 0..7, cls20.
That is 4 configs x 8 seeds = 32 train runs + 32 inference runs.
...
MODELS = ["multimodal", "sequential"]
ARMS = ["sdc", "agent"]          # "agent" adds --agent-centric
...
def main():
    ...
    print("       python3 run_grid_validate.py")
