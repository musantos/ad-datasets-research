...
def train(cls_weight, agent_centric, seed=None):
    ...
    print(f"[*] MULTIMODAL TRAINING (K={NUM_MODES}, cls_weight={cls_weight}, "
    ...
    log_file.write(f"# model=multimodal,cls_weight={cls_weight},arm={arm},"
    ...
    for epoch in range(EPOCHS):
        ...
        # improves it, overwriting a single multimodal_best.pth. The old
    ...
    print(f"\n[SUCCESS] Multimodal training finished (cls_weight={cls_weight}, arm={arm}).")
    print(f"Best checkpoint: multimodal_best.pth (epoch {best_epoch}, Val best mode: {best_val_reg:.4f})")
