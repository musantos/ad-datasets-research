...
# SIBLING of run_inference_map (V3 = social + map + lane topology). Changes vs V2,
# mechanical:
#   (1) dataset -> WaymoMotionDatasetMapTopo (10-tuple with map_adjacency);
#   (2) model   -> LaneTopoTrajectoryPredictor;
#   (3) unpack  -> 10-tuple; pass map_adjacency to the model;
#   (4) flags   -> --n-map-polylines / --n-points-per-polyline / --use-map-type
#       (MUST match training).
#
# THE agent->SDC INVERSION IS UNCHANGED FROM V0/V1/V2 -- and this is intentional,
# worth flagging for the write-up. The predictor outputs trajectories in the
# TARGET's agent-centric frame; to score them we invert back to the SDC/world
# frame using ONLY the target's pose@10 (p0, theta0). Neighbors, the map polylines
# AND the lane topology are INPUT CONTEXT ONLY: they condition the prediction but
# they never enter the coordinate inversion. The map/topology live in the same
# target frame purely so the encoder sees them aligned to the agent; the inverse
# transform is a rigid-body map of the OUTPUT and is a function of the target pose
# alone. So V3 adds topology to the CONDITIONING, not to the geometry of the
# inversion -- the inversion code below is identical across the whole ladder, and
# any V3-vs-V2 metric delta cannot come from a changed inversion.
...
CACHE_DIR = "/workspace/datasets/waymo/cache_val_map_topo"
...
CHECKPOINT_NAME = "lane_topo_best.pth"
...
def run_inference(tag, seed=None, standardize=False, n_neighbors=N_NEIGHBORS,
    ...
    standardize:      selects the _std checkpoint. Stats come from the checkpoint
                      ...
                      (V3 grid is raw-only; kept for parity.)
    n_neighbors:      MUST match training (social branch top-K).
    n_map_polylines:  MUST match training (map branch top-K). A different M also
                      changes the topology alignment -> train/inference mismatch.
    ...
    Agent-centric arm is IMPLICIT in V3 (arm='agent'):
        Checkpoint: checkpoints/lane_topo_<tag>_agent[_seed<n>][_std][_type]/lane_topo_best.pth
        Output:     predictions/lane_topo_<tag>_agent[_seed<n>][_std][_type]/
    """
    arm = "agent"
    ...
    dataset = WaymoMotionDatasetMapTopo(
    ...
    # Tolerant load of the standardization buffers (same logic as V0/V1/V2). A V3
    # checkpoint ALWAYS has the buffers (register_buffer in __init__), so the
    # 'missing' branch does not fire for V3 -- kept for consistency/defense. The
    # map modules (map_subgraph/map_proj/map_cross_attn), the lane_graph, and, when
    # use_map_type, map_type_emb ARE required (trained) and cannot be missing; a
    # mismatch of use_map_type vs the checkpoint surfaces here as
    # missing/unexpected -> raise.
    ...
    print(f"INFO: generating social+map+topology predictions for {len(by_file)} "
          f"scenarios ({len(dataset)} target agents)...")
    ...
    with torch.no_grad():
        for i, path in enumerate(sorted(by_file)):
            ...
            for idx, aid in by_file[path]:
                ...
                # mask + the 3 map tensors + the lane adjacency. 10-tuple; the
                # labels (y_future, future_mask, agent_type) are discarded here.
                (x_past, neighbors, nmask, map_polylines, map_type, map_mask,
                 map_adjacency, _, _, _) = dataset[idx]
                ...
                map_adjacency = map_adjacency.unsqueeze(0).to(device)  # [1,R,M,M]
                ...
                # agent->SDC INVERSION (IDENTICAL to V0/V1/V2): target pose@10
                # re-read from the same cache; neighbors, map AND topology do not
                # enter here (see the module header for why this is unchanged).
                #   forward:  xy_agent = (xy_sdc - p0) @ R.T
                #   inverse:  xy_sdc   =  xy_agent      @ R  + p0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generates vectorized+social+map+topology predictions over the validation split"
    )
    ...
    parser.add_argument("--use-map-type", action="store_true",
                        help="type ablation. MUST match training (else load_state_dict raises).")
