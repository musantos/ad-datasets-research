...
def run_inference(tag, agent_centric, seed=None):
    ...
    Paths (arm is folded into the tag so {sdc, agent} never collide):
        Checkpoint: checkpoints/sequential_<tag>_<arm>[_seed<n>]/sequential_best.pth
        Output:     predictions/sequential_<tag>_<arm>[_seed<n>]/
    """
    ...
    print(f"INFO: generating sequential predictions for {len(by_file)} scenarios "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generates sequential predictions over the validation split"
    )
