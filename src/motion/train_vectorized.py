...
def train(cls_weight, agent_centric, seed=None, standardize=False,
    ...
    print(f"[*] VECTORIZED TRAINING (K={NUM_MODES}, cls_weight={cls_weight}, "
    ...
    for epoch in range(EPOCHS):
        ...
        # improves it, overwriting a single vectorized_best.pth. The old
        ...
        if avg["reg"] < best_val_reg:
            ...
            torch.save(model.state_dict(),
                       os.path.join(checkpoint_dir, "vectorized_best.pth"))
            marker = "  <- best so far"
            is_best = 1
        else:
            epochs_no_improve += 1
    ...
    print(f"\n[SUCCESS] Vectorized training finished (cls_weight={cls_weight}, arm={arm}).")
    print(f"Best checkpoint: vectorized_best.pth (epoch {best_epoch}, Val best mode: {best_val_reg:.4f})")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Trains the vectorized K=6 model with Winner-Takes-All loss"
    )
